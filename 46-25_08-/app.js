...
const userElement = sect.querySelectorAll('.user');
userElement.forEach(u => u.onclick = userClickHandler);

function userClickHandler(e){
  left.innerHTML = '';
  right.innerHTML = '';
  const userId = +e.currentTarget.id.split('_')[1] //один т.к. отрезаем  user_ и остаётся 1.
  left.append(getAlbumsById(userId));
}
//================================Albums
//Structur sample
// const albums = [
...
function getPhotoByAlbumId(id) {
  const element = document.createElement('div');
  const photosByAlbumId = photos.filter(el => el.albumId === id);
  if (!photosByAlbumId.length) {
    element.innerHTML = 'No Photos'
    return element
  }
  photosByAlbumId.forEach(el => {
    element.append(renderPhoto(el))
  })
